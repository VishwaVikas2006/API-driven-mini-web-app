...
// 			if (showLatestOnly && res.data.results.length > 0) {
...
// 			fetchResults(1, true); // Only show latest result
// 			setKeyword('');
// 		} catch (err) {
...
function App() {
  ...
  const fetchResults = async (pageNum = 1) => {
    ...
    try {
      ...
      setError('Failed to fetch results');
      console.error('Fetch error:', err); // Extra logging
    }
    setLoading(false);
  };
  ...
  const handleSearch = async (e) => {
    ...
    try {
      await axios.post('/api/search', { keyword });
      fetchResults(1); // Show all results, remove showLatestOnly logic
      setKeyword('');
    } catch (err) {
      setError(err.response?.data?.error || 'API error');
